...
def get_income_approach_inputs():
    ...
    with st.form("Income Approach Form"):
        initial_revenue = st.number_input("Enter the initial annual revenue generated by the intangible asset (in INR)",
                                          min_value=0.0, key="income_revenue")
        growth_rate = st.number_input("Enter the annual growth rate (as a percentage, e.g., 5 for 5%)",
                                      min_value=0.0, key="income_growth") / 100
        discount_rate = st.number_input("Enter the discount rate (as a percentage, e.g., 10 for 10%)",
                                        min_value=0.0, key="income_discount") / 100
        useful_life = st.number_input("Enter the useful life of the intangible asset (in years)", min_value=1, step=1, key="income_life")
    ...
    if submitted:
        if initial_revenue > 0:
            ...
            st.session_state.income_value = income_value
            st.write(f"Estimated Value based on Income Approach: INR {income_value:,.2f}")
        else:
            st.session_state.income_value = 0
...
def get_market_approach_inputs():
    ...
    with st.expander("Market Approach Tips"):
        ...
        """)
    num_comparables = st.number_input("Enter the number of comparable transactions", min_value=0, step=1, key="num_comparables")
    comparables = []

    if num_comparables > 0:
        for i in range(num_comparables):
            with st.form(f"Comparable {i + 1} Form"):
                price = st.number_input(f"Enter the sale price of comparable {i + 1} (in INR)", min_value=0.0, key=f"price_{i}")
                adjustment = st.number_input(
                    f"Enter the adjustment for comparable {i + 1} (as a percentage, e.g., -10 for -10%)",
                    min_value=-100.0, key=f"adjustment_{i}") / 100
    ...
    if len(comparables) > 0:
        st.session_state.market_value = estimate_value_from_comparables(comparables)
    else:
        st.session_state.market_value = 0
...
def get_cost_approach_inputs():
    ...
    with st.form("Cost Approach Form"):
        reproduction_cost = st.number_input("Enter the reproduction cost of the intangible asset (in INR)",
                                            min_value=0.0, key="cost_reproduction")
        depreciation_rate = st.number_input("Enter the depreciation rate (as a percentage, e.g., 20 for 20%)",
                                            min_value=0.0, key="cost_depreciation") / 100
        submitted = st.form_submit_button("Calculate Cost Approach Value")

    if submitted:
        st.session_state.cost_value = calculate_cost_value(reproduction_cost, depreciation_rate)
        st.write(f"Estimated Value based on Cost Approach: INR {st.session_state.cost_value:,.2f}")
    else:
        st.session_state.cost_value = 0
...
def main():
    ...
    if 'market_value' not in st.session_state:
        st.session_state.market_value = 0
    if 'cost_value' not in st.session_state:
        st.session_state.cost_value = 0

    # Income Approach
    get_income_approach_inputs()

    # Market Approach
    get_market_approach_inputs()

    # Cost Approach
    get_cost_approach_inputs()

    # Combined Valuation
    combined_value = get_combined_valuation([st.session_state.income_value, st.session_state.market_value, st.session_state.cost_value])
    st.session_state.combined_value = combined_value

    # Display results in columns
    st.subheader("Valuation Results")
    col1, col2, col3 = st.columns(3)
    col1.metric("Income Approach", f"INR {st.session_state.income_value:,.2f}")
    col2.metric("Market Approach", f"INR {st.session_state.market_value:,.2f}")
    col3.metric("Cost Approach", f"INR {st.session_state.cost_value:,.2f}")
    st.metric("Combined Estimated Value", f"INR {st.session_state.combined_value:,.2f}")

    # Visualization
    st.subheader("Valuation Comparison")
    valuation_methods = ["Income Approach", "Market Approach", "Cost Approach"]
    valuation_values = [st.session_state.income_value, st.session_state.market_value, st.session_state.cost_value]
    ...
    if st.session_state.combined_value > 0:
        st.write("Valuation Summary")
        st.dataframe(valuation_data)

    # Provide option to download results
    st.subheader("Download Results")
    valuation_df = create_valuation_dataframe(st.session_state.income_value, st.session_state.market_value, st.session_state.cost_value, st.session_state.combined_value)
...
# Make sure the Streamlit app runs the main function
